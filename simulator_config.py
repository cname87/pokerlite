...
dealer_open_strategy_list = generate_possible_lists(max_len_strategies, "HML")
...
dealer_see_or_raise_after_non_dealer_opens_strategy_list = generate_possible_lists(max_len_strategies, "HMS")

# Possible strategies for the dealer when the non-dealer raises following a dealer open - see (or fold)
dealer_see_after_non_dealer_raises_strategy_list = generate_possible_lists(max_len_strategies, "S")

# Possible strategies for non-dealer when the dealer checks instead of opening - open high, medium or low, (or check)
non_dealer_open_after_dealer_checks_strategy_list = generate_possible_lists(max_len_strategies, "HML")

# Possible strategies for the non-dealer when the dealer opens - raise high or low, see, (or fold)
non_dealer_see_or_raise_after_dealer_opens_strategy_list = generate_possible_lists(max_len_strategies, "HMS")

# Possible strategies for the non-dealer when the dealer raises following a non-dealer open (after dealer check) - see (or fold)
non_dealer_see_after_dealer_raises_strategy_list = generate_possible_lists(max_len_strategies, "S")
